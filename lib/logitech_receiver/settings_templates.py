...
def _feature_mx_vertical_dpi_sliding():
    ...
    class _DpiSlidingRW(object):
        ...
        class MxVerticalDpiState(object):
            __slots__ = ('device', 'pressedCids', 'dpiSetting', 'dpiChoices', 'fsmState', 'dpiSlots', 'dpiSlotChosen', 'dx')
            ...
            def __init__(self, device):
                ...
                If it's released quickly, we just swap DPI slots.
            ...
            def calcMovedDpiIdx(self):
                currIdx = self.dpiSlots[self.dpiSlotChosen]
                idxDiff = int(self.dx / 15.)
                return min(max(currIdx + idxDiff, 0), len(self.dpiChoices) - 1)
            ...
            def handle_keys_event(self, cids):
                if self.fsmState == 'idle':
                    ...
                    if _special_keys.CONTROL.DPI_Switch not in cids:
                        self.setNewDpi(self.calcMovedDpiIdx())
                        self.fsmState = 'idle'
            ...
            def handle_move_event(self, dx, dy):
                ...
                if self.fsmState == 'pressed':
                    ...
                    self.dx += dx  # accumulate displacement
                    self.displayNewDpi(self.calcMovedDpiIdx())
